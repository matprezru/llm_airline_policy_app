...
class DocumentReader():
    ...
    def __init__(self, data_paths: Union[List, str]) -> None:
        ...
        """
        self.data_paths: List[str] = data_paths if isinstance(data_paths, list) else [data_paths]
    ...
    def _get_files_list(self) -> List[str]:
        ...
        for path in self.data_paths:
            ...
            # If the path corresponds to a directory, get all the internal files and append them to the list
                ...
                files = [os.path.abspath(os.path.join(path, f)) for f in os.listdir(path) if (os.path.isfile(os.path.join(path,f)) and not f.startswith('.'))]
    ...
    def read_documents(self) -> List[List[Document]]:
        ...
        for file_path in self.file_paths:
            ...
            if ext == ".pdf":
                ...
                logger.warning(f"Could not load file '{file_path}'. Unsupported file type: '{ext}'")
...
class Loader(ABC):
    ...
    def _add_metadata(self, doc: Document, extension: Optional[str] = None, file_path: Optional[str] = None):
        ...
        """
        if file_path:
            if not "source" in doc.metadata.keys():
...
class PdfLoader(Loader):
    ...
    def load_file(self, file_path: str) -> List[Document]:
        ...
        for i, page in enumerate(document_pages):
            document_pages[i] = self._add_metadata(doc=page, extension=".pdf", file_path=file_path)
...
class MdLoader(Loader):
    ...
    def load_file(self, file_path: str) -> List[Document]:
        ...
        # Add metadata fields
        document_object = self._add_metadata(doc=document_object, extension=".md", file_path=file_path)
