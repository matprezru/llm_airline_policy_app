...
class VectorDB():
    ...
    def __init__(self, persist_dir: str) -> None:
        ...
        self.embeddings = CustomEmbeddings(provider = os.getenv("EMBEDDINGS_PROVIDER"), model_name = os.getenv("EMBEDDINGS_MODEL"))
        self.db = Chroma(
    ...
    def index_documents(self, documents: List[List[Document]]) -> str:
        ...
        logger.info(f"There are {len(documents)} documents to be indexed in the vector database.")
        logger.info(f"There are {len(chunks)} chunks to be indexed in the vector database.")
        ...
        if new_chunks:
            ...
            uploaded_ids = self.db.add_documents(documents=new_chunks, ids=new_chunk_ids)
            if uploaded_ids:
                message = f"{len(uploaded_ids)}/{len(new_chunks)} chunks have been uploaded successfully"
                logger.info(message)
            else:
                raise Exception("The documents could not be indexed in the vector database.")
        else:
